...
function cleanFeed() {
  // Select all posts
  document.querySelectorAll('div.feed-shared-update-v2, div.update-components-feed-update').forEach((post) => {
    ...
    if (post.innerText.includes('Suggested for you') || post.innerText.includes('Suggested')) {
      post.remove();
      return;
    }
    ...
    const timeElement = post.querySelector('span.update-components-actor__sub-description, span.feed-shared-actor__sub-description');
    if (timeElement) {
